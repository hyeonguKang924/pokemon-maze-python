...
imgPokemon = pygame.image.load("image/Pokemon0.png")
imgpikapika = pygame.image.load("image/pikapika.png")
imgpoketball2 = pygame.image.load("image/poketball2.png")
imgItem = [
    pygame.image.load("image/potion.png"),
    pygame.image.load("image/emerald.png"),
...
imgFloor = [
    ...
    pygame.image.load("image/poketball.png"),
...
COMMAND = ["[A] 100,000 volts", "[P] Potion", "[E] Emerald", "[R] Run"]
TRE_NAME = ["Potion", "emerald", "Food spoiled.", "Food +20", "Food +100"]
EMY_NAME = [
    "Eevee", "Pidgeotto", "Cubone", "Ogre", "Slowking",
    "Charizard", "Dugtrio", "Dragonite", "Mewtwo", "Arceus"
]
...
def move_player(key):  # 주인공 이동
    global idx, tmr, pl_x, pl_y, pl_d, pl_a, pl_life, food, potion, emerald, treasure
...
def draw_para(bg, fnt):  # 주인공 능력 표시
    ...
    draw_text(bg, str(emerald), X + 266, Y + 33, fnt, WHITE)

def init_battle():  # 전투 시작 준비
    global imgPokemon, emy_name, emy_lifemax, emy_life, emy_str, emy_x, emy_y
    typ = random.randint(0, floor)
    if floor >= 10:
        typ = random.randint(0, 9)
    lev = random.randint(1, floor)
    imgPokemon = pygame.image.load("image/Pokemon" + str(typ) + ".png")
    emy_name = EMY_NAME[typ] + " LV" + str(lev)
    emy_lifemax = 60 * (typ + 1) + (lev - 1) * 10
    emy_life = emy_lifemax
    emy_str = int(emy_lifemax / 8)
    emy_x = 440 - imgPokemon.get_width() / 2
    emy_y = 560 - imgPokemon.get_height()
...
def draw_battle(bg, fnt):  # 전투 화면 표시
    ...
    if emy_life > 0 and emy_blink % 2 == 0:
        bg.blit(imgPokemon, [emy_x, emy_y + emy_step])
...
def main():  # 메인 처리
    ...
    global pl_a, pl_lifemax, pl_life, pl_str, food, potion, emerald
    ...
    while True:
        ...
        if idx == 0:  # 타이틀 화면
            ...
            screen.blit(imgpikapika, [450, 200])
            screen.blit(imgpoketball2, [170, 330])
            if fl_max >= 2:
            ...
            if tmr == 5:
                set_message(emy_name + " 100,000 volts!")
                se[0].play()
                emy_step = 30
            if tmr == 9:
